...
- "source" (optional): string, where this grid's coordinates came from — the one place
  ...
  - It identifies the construction; it is not always a command you can paste back.
    A gridId, gridName or extra category given on the command line doesn't appear
    there, so `data/P6.json` reads "genPrism.py 6" while the run that made it also
    passed "Hexagonal prism" and gained a "parallelohedron" category. So regenerate
    from `source` only after checking what the stored metadata says, or you will
    quietly lose it.
