...
def build(primal_path, grid_id=None, grid_name=None, categories=None):
    ...
    # The command that makes this one, spelled out: --all-catalan writes thirteen, and
    # the primal's path is the argument that decides which. See
    # json_format.source_line.
    grid['source'] = json_format.source_line([f'data/{Path(primal_path).name}'])
