...
def main():
    ...
    # So the file says where it came from, and can be reproduced exactly. "source"
    # rather than a vague "_comment": provenance is a specific thing, and an
    # imported grid records the same thing as a URL (see docs/json-format.md).
    # From argv rather than rebuilt from the parameters: the hand-built version here
    # left out the grid id and name when those were given on the command line, so a
    # grid named that way claimed a source that would not have named it.
