/**
 * author: Lee Stemkowski
 * adapted by Lars Huttar
 * Class for a surface in 3D, e.g. of a polyhedron.
 */
...
// splice elements not already in array
...
// replace all values equal to oldData with newData
...
// Find ID of edge whose vertices are va, vb
...
// Find ID of edge whose faces are fa, fb
...
// Compute and store center of each edge and face.
...
// Get a string description of an edge, vertex, or face.
...
// create topology from minimal polyhedron data as found in polyhedron_data.js
// I have tested this code using POLYHEDREN_DATA.Cube, and it seems to be working
// fine, including added incidence data.
TOPOLOGY.createFromPolyhedronData = function( data )
{
	// initialize topology
	var topo = new TOPOLOGY.Topology();

	// add vertices
	for (var vertexIndex = 0; vertexIndex < data.vertex.length; vertexIndex++)
	{
                var dataV = data.vertex[vertexIndex];
		var topoV = new TOPOLOGY.Vertex( {ID: vertexIndex,
		        vector3: new THREE.Vector3(dataV[0], dataV[1], dataV[2]) } );
		topo.add(topoV);
	}
	
	// add faces, link vertex-face IDs 
	for (var faceIndex = 0; faceIndex < data.face.length; faceIndex++)
	{
		var faceVertexIDs = data.face[faceIndex];
		
		var f = new TOPOLOGY.Face( {ID: faceIndex} );
		topo.add(f);
		topo.addIncidenceData( "face", faceIndex, "vertex", faceVertexIDs);
	}
	...
	for (var faceIndex = 0; faceIndex < data.face.length; faceIndex++)
	{
		...
		// indices of vertices on the face
        	for (var vertexIndex = 0; vertexIndex < faceVertexIDs.length; vertexIndex++) {
        	        var vID1 = faceVertexIDs[vertexIndex];
        	        var vID2 = faceVertexIDs[
        	                vertexIndex + 1 >= faceVertexIDs.length ? 0
        	                        : vertexIndex + 1];
        	        edgeArray.push([vID1, vID2]);
		}
        ...
        console.log(topo); // for debugging
